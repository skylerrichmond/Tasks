...
unique(beren3$event)
Self Quiz: The graph that came from the code on eCampus is easier to understand as the information is placed with a different time scale on the x axis changing the viewing window to make it easier. The one on ecampus has the same data essentially but it being put into days instead of time od the day made it much clearer. 
...
avgWet<- mean(beren3$value[Wet])
sum(Wet)
sum(Feeds)
sum(Wet, Feeds)
totalWet<-sum(Wet,Feeds)
avgWet<- tapply(beren3$value[Wet], beren3$event[Feeds], mean)
varWet<-tapply(beren3$value[Wet], beren3$event[Feeds], var)
totalWet<- tapply(beren3$value[Wet], beren3$event[Feeds], sum)
numWet<-tapply(beren3$value[Wet], beren3$event[Feeds], length)
or(beren3$value[Wet], beren3$event[Feeds])
cor.test(beren3$value[Wet], beren3$event[Feeds])
berenCor<- cor.test(beren3$value[Wet], beren3$event[Feeds])
summary(berenCor)
berenANOVA <- aov(beren3$value[Wet] ~ beren3$event[Feeds])
boxplot( beren3$value[Wet] ~ beren3$event[Feeds], xlab=" number of bottles", ylab= " amount of diapers" )
par(las=1, mar=c(5, 5, 1, 1), mgp=c(2, 0.5, 0), tck=-0.01 )
plot(as.numeric(names(totalWet)), totalWet, type="b", pch=16, xlab="number of bottles", ylab="amount of diapers")
abline(h=mean(totalWet), lty=2, col='red')
pdf("r02c-totalWetByBottle.pdf", height =4, width=4)
par(las=1, mar=c(5, 5, 1, 1), mgp=c(2, 0.5, 0), tck=-0.01)
dev.off()
totalBottle<-sum(Feeds)
totalWet<-sum(Wet)
plot(totalBottle,totalWet)
